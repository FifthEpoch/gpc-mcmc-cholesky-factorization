...
"""
Matrix abstractions for positive semidefinite (PSD) operators and kernel matrices.
    ...
    def _clean_index_input(self, args):
        ...
        for j in range(2):
            if isinstance(idx[j], np.ndarray):
                ...
                raise RuntimeError(
                    f"Indexing not implemented with index of type {type(idx[j])}"
                )
...
class KernelMatrix(FunctionMatrix):
    ...
    def kernel_from_input(kernel, bandwidth=1.0, extra_stability=False, **kwargs):
        if isinstance(kernel, str):
            if kernel == "gaussian":
                ...
                raise RuntimeError(f"Kernel name {kernel} not recognized")
        else:
            return kernel
    ...
    def __init__(self, X, kernel="gaussian", bandwidth=1.0, **kwargs):
        ...
        kernel, kernel_vec, kernel_mtx = KernelMatrix.kernel_from_input(
            kernel, bandwidth=self.bandwidth, **kwargs
        )
        self.kernel = kernel
...
class NonsymmetricKernelMatrix(object):
    def __init__(self, X, Y, kernel="gaussian", bandwidth=1.0, **kwargs):
        ...
        (
            ...
            self.kernel_vec,
            self.kernel_mtx,
        ) = KernelMatrix.kernel_from_input(kernel, bandwidth=bandwidth, **kwargs)
