...
"""
Low-rank representations of PSD matrices (Nyström and related forms).

Adapted from the Randomly-Pivoted-Cholesky reference implementation.
"""
...
class CompactEigenvalueDecomposition(AbstractPSDLowRank):
    ...
    def krr_vec(self, b, lam):
        ...
        return b / lam + self.V @ (
            np.divide(VTb, diag[:, np.newaxis]) - VTb / lam
        )
...
class PSDLowRank(AbstractPSDLowRank):
    ...
    def eigenvalue_decomposition(self):
        return CompactEigenvalueDecomposition.from_G(
            self.G, idx=self.idx, rows=self.rows
        )
...
class NystromExtension(AbstractPSDLowRank):
    ...
    def get_core_factor(self):
        if self.L is None:
            self.L = np.linalg.cholesky(
                self.C
                + np.trace(self.C) * np.finfo(float).eps * np.identity(self.C.shape[0])
            )
        return self.L
    ...
    def scale(self, scaling):
        ...
        return NystromExtension(
            self.C,
            idx=self.idx,
            rows=self.rows * scaling[np.newaxis, :],
            factor=G,
            interpolation=self.W,
        )
