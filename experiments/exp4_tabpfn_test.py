"""
Experiment 4: TabPFN tabular foundation model baseline (local).

Uses the local TabPFN package as a classifier on the same frozen embeddings
produced by extract_embeddings.py. Model weights are downloaded automatically
on first use and cached locally (under ~/.cache/tabpfn by default), so this
script does not require an API key or network access at inference time.

TabPFN is pretrained on datasets of up to ~10k samples. For larger training
sets the script subsamples to --max-train-samples (default 3500). Pass
--ignore-pretraining-limits to let TabPFN run on training sets that exceed
its recommended ceiling.
...
def _resolve_device(requested: str) -> str:
    """Map requested device string to an actually-available torch device."""
    try:
        import torch
    except ImportError:
        # tabpfn depends on torch, so this should not happen in practice.
        return requested
...
def run_experiment(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        # Older TabPFN releases don't expose every kwarg; fall back gracefully.
        clf_kwargs.pop("ignore_pretraining_limits", None)
        clf_kwargs.pop("n_estimators", None)
        clf = TabPFNClassifier(**clf_kwargs)
    ...
    metrics["tabpfn_backend"] = "tabpfn_local"
    metrics["device"] = device
    metrics["n_estimators"] = int(args.n_estimators)
    metrics["ignore_pretraining_limits"] = bool(args.ignore_pretraining_limits)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Exp4: local TabPFN tabular model baseline.")
    ...
    parser.add_argument("--max-train-samples", type=int, default=3500,
                        help="Subsample train set to this size before TabPFN fit")
    parser.add_argument("--predict-chunk-size", type=int, default=1000,
                        help="Chunk size for batched predict_proba calls")
    parser.add_argument("--device", type=str, default="cuda",
                        help="Torch device for local TabPFN ('cuda' or 'cpu')")
    parser.add_argument("--n-estimators", type=int, default=4,
                        help="Number of ensemble members for local TabPFN (default 4)")
    parser.add_argument("--ignore-pretraining-limits", action="store_true",
                        help="Allow training sets larger than TabPFN's pretrained recommendation")
